...
def find_markets(location_data,  city, state, zip_code, max_miles=10):
    ...
    if len(indexes_of_comparing_markets)==0:
        ...
        pass
    market_lat, market_lon = location_data[indexes_of_comparing_markets[0]][5],location_data[indexes_of_comparing_markets[0]][6]
...
def option_find_markets(FMID_codes,media_codes,location_codes,payment_codes,season_codes,products_codes,limited_ids):
    flag_close = False
    flag_page = False
    flag_info = False
    flag_sort = False
                    ...
                    if request == "Close":
                        ...
                        if request.isdigit():
                            ...
                            if not flag_out_of_page:
                                view.print_all_info_about_market_by_index(FMID_codes,media_codes,location_codes,payment_codes,season_codes,products_codes,data[int(request)+page*10 -1][0],limited_ids)
                                flag_info = True
                            else:
        ...
        if request == "2":
            ...
            if flag_login:
                ...
                if flag_error:
                    ...
                    zip_users.save_market_review(username,str(last_request-1+last_page*LIMITED_IDS_ON_PAGE),grade,user_review)
            ...
            if flag_login:
                ...
                if index.isdigit() and int(index)<=LIMITED_IDS_ON_PAGE:
                    if market_reviews[int(index)-1 + LIMITED_IDS_ON_PAGE*page][0] == username or flag_user_admin:
                        zip_users.delete_market_review(username,int(last_request)-1 + LIMITED_IDS_ON_PAGE*last_page)
...
def sorting_menu_info(all_info,sort_id):
    ...
    if destination == "1":
        ...
        FMID_codes,media_codes,location_codes,payment_codes,season_codes,products_codes = zip_util.decomposition(sorted_data)
    elif destination == "2":
...
def all_markets_view(FMID_codes,media_codes,location_codes,payment_codes,season_codes,products_codes,limited_ids,limited_pages,flag_login,flag_user_admin,username,all_info):
    page = 0 
    flag_close = False
    flag_info = False
    flag_page = False
        ...
        if not flag_review:
            if not flag_sort:
                    ...
                    if request == "Close":
                        ...
                        if request.isdigit():
                            ...
                            if not flag_out_of_page:
                                ...
                                view.print_all_info_about_market(FMID_codes,media_codes,location_codes,payment_codes,season_codes,products_codes,int(request),limited_ids,page,grade_of_market)
                ...
                if request == "2":
                    ...
                    all_info,FMID_codes,media_codes,location_codes,payment_codes,season_codes,products_codes = sorting_menu_info(all_info,request)
                    flag_sort = False
                else:
                    view.print_unknown_command()
                    flag_sort = False
            ...
            flag_review,flag_close,reviews = in_review_info(reviews,flag_login,flag_user_admin,username,page,last_request)
...
while not Flag_end:
    ...
    if request == "5":
        ...
        all_info,FMID_codes,media_codes,location_codes,payment_codes,season_codes,products_codes = all_markets_view(FMID_codes,media_codes,location_codes,payment_codes,season_codes,products_codes,limited_ids,limited_pages,flag_login,flag_user_admin,username,all_info)
        # zip_code = input()
        # print(zip_code)
        ...
        option_find_markets(FMID_codes,media_codes,location_codes,payment_codes,season_codes,products_codes,limited_ids)
    #     print(request)
